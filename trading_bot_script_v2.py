...
def get_balance():
    ...
    balance = float(response['result']['BTC']['available_balance'])
    
    price = session.latest_information_for_symbol(symbol="BTCUSD")
    adj_price = float(price['result'][0]['last_price'])
    
    avail_balance = adj_price * balance
    adj_balance = avail_balance * 0.97
...
adj_balance = get_balance()

def make_trade(prediction, adj_balance):
